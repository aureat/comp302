...
class MapTerritoryTest1 {
    @Test
    void testMousePressedInDraftPhaseAndOwner() {
        Fonts.register();
        ...
        mapTerritory.mousePressed(mouseEvent);
        if(state.getOwner() == game.getCurrentplayer()) {
            assertEquals(armies + 1, state.getArmies());
            assertEquals(draftArmies - 1, game.getDraftArmies());
        }
        assertTrue(repOk(mapTerritory));
    }
